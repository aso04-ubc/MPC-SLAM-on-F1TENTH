...
namespace AEB {
    ...
    void DepthMapSafetyInfoProvider::OnInit(rclcpp::Node *node) {
        m_Node = node;
        // Create either a host (CPU) or a CUDA-backed implementation depending on compute mode.
        m_DepthMapToBucketComputer = IDepthMapToBucketComputer::Create(m_ComputeMode);

        RegionOfInterestInfo regionOfInterestInfo{};
        // ROI parameters are in pixel coordinates.
        // Implementations treat them as half-open intervals: [StartX, EndX) x [StartY, EndY).
        ...
        // Distances and histogram step are configured in millimeters.
    ...
    void DepthMapSafetyInfoProvider::OnDepthMap(sensor_msgs::msg::Image::SharedPtr msg) {
        ...
        // The implementation assumes the depth image is:
        // - 16-bit unsigned single channel ("16UC1")
        // - little-endian
        // In practice this usually means each pixel stores depth in millimeters.
        ...
        // Reinterpret the raw byte buffer as an array of uint16 depth values.
        // (The caller must ensure msg->data has the expected size.)
        ...
        if (result) {
            ...
            // `result` is in millimeters; convert to meters for SafetyInfo.
            ...
            if (speed > 0) {
                // TTC ~= distance / speed (constant-speed approximation).
                // Units: (m) / (m/s) = seconds.
