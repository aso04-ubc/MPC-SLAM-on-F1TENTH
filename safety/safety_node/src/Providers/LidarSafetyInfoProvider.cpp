...
namespace AEB {
    ...
    void LidarSafetyInfoProvider::OnLaserScan(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
        ...
        // Only consider scan rays close to the current steering direction.
        // Units: radians (same unit as LaserScan angle_min / angle_increment).
        const double angle_tolerance = 0.53;
        for (size_t i = 0; i < msg->ranges.size(); ++i) {
            double r = msg->ranges[i];

            // Skip invalid rays and anything outside the sensor's supported range.
            ...
            // Compare ray angle to current steering angle; LaserScan provides angles in a fixed frame.
            if (std::abs(angle-current_steer) > angle_tolerance) continue;

            min_distance = std::min(min_distance, r);

            // Approximate closing rate along this ray.
            // TTC is only computed when the closing rate is positive (range_rate > 0).
            double range_rate = v * std::cos(angle);

            if (range_rate > 1e-3) {
                // TTC = distance / closing_speed, units: seconds.
