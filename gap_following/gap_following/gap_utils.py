...
class GapFollowAlgo:
    ...
    This class implements a cost-based gap selection algorithm designed for smooth
    driving and effective U-turn handling. It preprocesses LiDAR data, applies
    obstacle inflation (disparity extender), and selects the optimal driving
    direction based on configurable cost functions.

    Key features:
    - LiDAR range preprocessing with smoothing and filtering
    - Obstacle inflation based on vehicle width and disparity detection
    - Cost-based gap selection with forward bias and wall avoidance
    - Emergency near-wall recovery behavior
    - Adaptive smoothing based on clearance distance
    """
    ...
    def __init__(self, max_range=3.5, min_safe_distance=0.25, car_width=0.5, disparity_threshold=1.2, smoothing_window_size=10):
        ...
        Args:
            max_range: Maximum distance considered for gap finding (meters).
                      Distances larger than this are saturated to detect open space/U-turns.
            min_safe_distance: Conceptual threshold for how close we want to get to obstacles.
                             Used for parameter validation but not directly in gap selection.
            car_width: Physical width of the vehicle (meters).
                      Used to compute obstacle inflation angle for safe gap detection.
            disparity_threshold: Minimum jump in consecutive ranges to classify as obstacle edge.
                               Smaller values detect more obstacles, larger values are more permissive.
            smoothing_window_size: Size of moving average kernel for LiDAR smoothing.
                                 Larger windows provide smoother data but more lag.
        """
    ...
    def process_lidar_and_find_gap(self, ranges, angle_min, angle_increment):
        """
        Process LiDAR data and find the optimal driving gap.

        This is the main algorithm entry point that:
        1. Preprocesses the LiDAR ranges (filtering, smoothing)
        2. Applies disparity extender to inflate obstacles
        3. Handles emergency near-wall recovery if needed
        4. Performs cost-based gap selection for normal driving
        5. Applies adaptive smoothing based on clearance

        Args:
            ranges: Array of LiDAR range measurements (numpy array)
            angle_min: Starting angle of the scan in radians
            angle_increment: Angular increment between measurements in radians

        Returns:
            tuple: (target_angle, best_idx)
                   target_angle: Optimal steering angle in radians
                   best_idx: Index of the best gap in the processed ranges array
        """
        ...
        #used for cost function graph visualization
    ...
    def _preprocess_ranges(self, ranges, angle_min, angle_increment):
        ...
        Applies several filtering and cleaning steps to prepare LiDAR data:
        - Replaces NaNs/Infs with bounded numeric values
        - Applies moving-average smoothing (minimum with smoothed to avoid creating false space)
        - Enforces physical limits on distance values
        - Filters out extremely small ranges (likely noise/self-hits)
        - Restricts processing to symmetric front 180° field of view

        Args:
            ranges: Raw LiDAR range measurements (numpy array)
            angle_min: Starting angle of the scan in radians
            angle_increment: Angular increment between measurements in radians

        Returns:
            numpy.ndarray: Processed range array, filtered to front 180° FOV
        """
    ...
    def _apply_disparity_extender(self, ranges, angle_increment):
        ...
        Detects sudden jumps (disparities) in consecutive range measurements that
        indicate obstacle edges. For each disparity, inflates the closer obstacle
        side by extending the range limitation laterally over neighboring beams.
        This creates "bubbles" around obstacles that account for the vehicle's
        physical width, preventing the planner from attempting to drive through
        gaps that are too narrow.

        Args:
            ranges: Preprocessed LiDAR range array
            angle_increment: Angular increment between measurements in radians

        Returns:
            numpy.ndarray: Range array with obstacles inflated to respect car width
        """
